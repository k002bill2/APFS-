...
| 2026-07-12 | `block-main-write.sh`에 삭제 push 예외 추가 — main/master에서도 브랜치 삭제 push(`--delete`/`-d`/`:refspec`)는 통과, 단 삭제 대상이 main/master면 계속 차단(2단 게이트) | `.claude/hooks/block-main-write.sh` | 병합된 피처 브랜치 정리 시 `git push origin --delete <branch>`가 배포와 무관한데도 훅에 오탐 차단되던 마찰 해소. 삭제 push는 커밋을 얹지 않아 Vercel 배포를 유발하지 않음. 검증: stdin JSON 합성 10-케이스 매트릭스(피처 삭제 4 통과 / main 콘텐츠·main 삭제·커밋 6 차단) 전부 green |
