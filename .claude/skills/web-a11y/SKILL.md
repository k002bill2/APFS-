...
- **B. 복합 컨트롤을 `<label>`로 감싸면 오작동.** Tiptap 리치텍스트·FilePond처럼 내부에 자체 버튼을 품은 컨트롤을 `<label>`로 감싸면, 암묵 연결이 라벨 불가한 본문(`div[role=textbox]`)을 건너뛰고 **툴바 첫 버튼(B/굵게)** 과 연결된다 → 본문 클릭이 `toggleBold` 발화(빈 문단에 bold 박힘). → `Field`의 `plain` 옵션(div 래핑) + **컨트롤이 자체 `aria-label` 보유**. 네이티브 단일 컨트롤(input·select)만 `<label>` 암묵 연결을 쓴다. (근거: generic_list_modal.tsx:41-46)
