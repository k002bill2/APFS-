...
- **E. 색만으로 정보 전달 금지.** StatusBadge·경고는 색+**텍스트/아이콘** 병행(색맹 대응). tone만 다르고 라벨이 같으면 구분 불가.
...
8. **색+텍스트 병행**(함정 E), **대비**는 color-tokens 준수, **모션**은 전역 reduced-motion에 얹되 JS 자동이동은 직접 가드.
