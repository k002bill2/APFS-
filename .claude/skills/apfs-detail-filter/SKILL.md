...
- **적용 칩은 GridFrame `appliedFilters` 로 넘긴다**(2026-09-24 — 툴바 아래 둘째 줄, 렌더는 `applied_filters.tsx` 단일 소유 → [[apfs-grid]] "필터 툴바 2줄 규약"). 아래 규칙(개별 칩·값만·×)은 그 컴포넌트가 구현한다 — 페이지에서 칩 마크업을 다시 쓰지 않는다.
...
- **모드형 조회기준이 있으면 그것이 주 필터다**(2026-09-24 사용자 지시, `invest_recovery_detail.tsx`): 컬럼·데이터를 함께 바꾸는 `조회기준`(투자및회수/전체거래)은 SegTabs 가 아니라 **깔때기 아이콘 바로 뒤 첫 칩 줄**의 `FilterChip`(건수 = 그 모드 행에 현재 조건을 건 facet)으로 둔다. 나머지 원문 검색항목(계정구분 등)은 드로어 + 적용 칩(`appliedFilters` 둘째 줄)으로만 — 첫 줄에 두 번째 칩 무리를 만들지 않는다.
