...
5. **입력 폰트 = 14px (프로젝트 표준)**: 모든 `input`/`select`/`textarea`는 **14px로 통일** — 공유 스타일 객체에 `fontSize:14` 한 곳(예: `generic_list.drawerInputStyle`·`schemas/renderers.SchemaField`·`asset_funding_aggrid.inputStyle`). ⚠️ 14px(<16px)라 **iOS Safari는 포커스 시 자동 확대**되지만, 프로젝트가 폼 폰트 14px 통일을 우선해 이 트레이드오프를 **의도적으로 수용**(데스크톱 우선 업무화면). 모바일 Safari가 1차 타깃이 되는 화면에서만 해당 입력을 16px로 상향.
...
- **F. iOS 입력 줌(14px)** → 프로젝트가 14px 통일 위해 **수용한 동작**(체크리스트 5). 모바일 Safari 우선 화면에서만 해당 입력을 16px로 상향.
...
2. 브라우저에서 **1280 / 768 / 400(데스크톱 창은 ~500까지 클램프됨)** 폭으로 측정:
   ...
   - **입력 폰트** — `getComputedStyle(input).fontSize === "14px"`(프로젝트 표준).
...
| 증상 | 진짜 원인 | 처방 |
|------|-----------|------|
| iOS에서 입력 포커스 시 화면 확대 | 입력 폰트 14px(<16px) | 프로젝트가 14px 통일 위해 **의도 수용** — 모바일 Safari 우선 화면에서만 16px |
