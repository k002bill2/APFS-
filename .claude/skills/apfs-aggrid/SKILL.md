...
- **체크박스는 "선택이 액션을 만들 때"만 만든다(기본값 아님).** 체크로 실행할 것이 있는 화면 — 다건 선택삭제, 단계 전이(→[[apfs-stage-workflow]]), 선택 행 편집 — 만 `checkboxes:true`. **조회 전용(감사·이력·집계)처럼 선택이 아무 것도 못 하는 화면은 `checkboxes:false`** 로 두고 `selectionColumnDef`도 **지운다**(체크박스가 꺼지면 선택 컬럼 자체가 생성되지 않아 죽은 설정이 된다). 선례: `audit_log.tsx`·`permission_history.tsx`(2026-09-15 제거) · `risk_manage.tsx` · `asset_funding.tsx`.
