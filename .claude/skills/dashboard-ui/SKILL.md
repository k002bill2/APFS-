...
- 이유: 자체 SVG는 번들 0KB에 CSS 변수 토큰·다크모드 연동이 완비돼 있다. 외부 라이브러리(특히 canvas 렌더)는 CSS 변수를 못 읽어 이 연동을 전부 다시 뚫어야 한다.
...
- **모션 토큰**: duration은 `duration-tok-fast`/`duration-tok`(=`--dur-*` CSS 변수), easing은 `ease-ds`로 통일. 신규 리터럴 `duration-150` 금지. indeterminate 등 keyframe 애니메이션은 **저모션 폴백을 함께 정의**(0% 프레임이 화면 밖이면 "멈춤"으로 오독됨). **소유자 없는 모션**(숫자 팝인·텍스트 스왑·성공 체크·에러 shake·shimmer·스태거 리빌·카운터 등)의 레시피는 [[transitions-dev]](벤더링, 32종)에서 고르고 [[transitions-polish]]로 튠한다 — 단 Radix 오버레이·sonner·Skeleton·switch/checkbox는 여기 규약이 소유하므로 그 스킬을 적용하지 않는다(스킬 첫 절 "APFS 적용 규약").
...
- **`StatusBadge` 옵션**: `size="sm"|"md"|"lg"`(11/12/13px). ⛔ **뱃지는 텍스트만 — 앞 점(bullet) 없음**(2026-09-24 사용자 결정: `dot` 옵션 자체를 삭제, 전 화면 일괄 제거. 가드 `src/dash/status_badge_no_dot.test.ts`). 점을 되살리거나 손으로 `rounded-full` 점을 붙이지 않는다 — 아이콘이 꼭 필요하면 `icon` prop만. **그리드 셀 뱃지는 `lg`(13px)로 통일**(스키마 렌더러 `status` 셀 포함). 톤 7종 `primary|success|warning|danger|info|cyan|muted` — **중립(비활성·대상아님)은 `muted`**(info는 "진행 중/정보"라 중립 대용 금지). 배지가 촘촘히 반복되는 열(심사단계)은 `lg`가 확정 규격.
- **메뉴 항목 슬라이드 하이라이트(전 메뉴 공통)**: 드롭다운·컨텍스트·커스텀 메뉴의 항목 hover/focus 배경은 **모두** `src/dash/ui/menu-highlight.tsx`의 공유 프리미티브를 쓴다(animate-ui MotionHighlight — layoutId 공유 span이 항목 간 spring으로 미끄러진다). **즉시배경(`focus:bg-*`·`hover:bg-*`·인라인 `background` 토글)을 새로 쓰지 말 것** — 슬라이드와 겹쳐 이중 배경이 된다. 배선 4단계: ① Content/열림영역을 `<MenuHighlightProvider>`로 감싼다(반드시 열림 조건 **안쪽** — 닫힘 시 unmount로 active 리셋). ② 각 항목은 **별도 컴포넌트**에서 `const {id,setActive}=useMenuHighlight()` 호출(`.map` 콜백에서 직접 훅 호출=규칙 위반). ③ Radix 메뉴는 `onFocus`만으로 hover·keyboard 커버(포인터 이동도 item.focus 유발), bespoke 버튼은 `onMouseEnter`·`onFocus` **둘 다**. blur/mouseleave에선 **지우지 않는다**(지우면 슬라이드가 스냅). ④ 버튼에 `relative isolate`, 안에 `<ItemHighlight id={id} danger={...}/>`, 구분선은 `relative z-10`. 소비 표면: `ui/dropdown-menu`·`ui/context-menu`·shell `HistoryMenu`(방문기록)·shell `FavoritesFab`(즐겨찾기)·`row_context_menu`. **새 커스텀 팝업 메뉴도 이 프리미티브를 쓴다(예외 없음).** ⚠️ **상주(unmount 안 됨)하는 네비게이션(LNB 좌측 메뉴)에는 쓰지 않는다** — 활성 페이지 지속 표시와 잔류 슬라이드가 겹쳐 "배경 여러 개"로 보인다(2026-09-11 LNB 적용→원복). 상주 메뉴는 **즉시 hover 배경**(`onMouseEnter`=`--muted`, `onMouseLeave`=활성이면 primary bg 복원·아니면 transparent) + 활성 리프 `aria-current`+primary. 검증은 반드시 **트러스티드 page.keyboard 화살표**로(프로그래밍 focus/dispatch는 가짜 성공) + 콘솔 렌더루프 무오류.
