...
- **`StatusBadge` 옵션**: `size="sm"|"md"|"lg"`(11/12/13px) · `dot={false}`(앞 점 제거). 톤 7종 `primary|success|warning|danger|info|cyan|muted` — **중립(비활성·대상아님)은 `muted`**(info는 "진행 중/정보"라 중립 대용 금지). 배지가 촘촘히 반복되는 열(심사단계)은 `lg`+`dot={false}`가 확정 규격.
