...
- **그리드 하단 선은 1개** — 마지막 행·합계 행의 `border-bottom` 은 지우고(`.apfs-section-head + * .ag-row-last` 투명), 경계선은 다음 SectionHead/푸터의 `border-top` 하나가 긋는다. `.apfs-grid-min` 최소 본문 높이는 **42px = 행 높이**(48px 이면 1행 그리드 아래 6px 빈 띠가 생겨 선 2개로 보였다). 가로 스크롤 띠가 있으면 `행 → 스크롤 띠 → 선` 순서가 정상이다.
