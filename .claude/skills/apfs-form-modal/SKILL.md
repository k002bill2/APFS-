...
7. **섹션 버튼 정렬 = 제목 바로 옆 좌측 (2026-09-24 사용자 결정).** 모달 본문 섹션(·소제목)에 딸린 버튼(`추가`·`행 추가`·`조회`·`저장` 등)은 **제목 텍스트 바로 뒤, 같은 줄 좌측**에 붙인다. 제목 줄 우측 끝으로 밀지 않는다(`ml-auto`·`justify-between` 금지), 섹션 본문 아래·표 아래로 따로 떼지도 않는다.
   ```tsx
   <div className="flex items-center gap-2 …섹션 헤더 클래스(규칙 3)…">
     <h3 …>{title}</h3>
     {actions && <div className="flex items-center gap-1.5">{actions}</div>}   {/* ml-auto 없음 */}
   </div>
   ```
   - 버튼은 `UI.Button size="sm"`, 추가류는 `variant="outline" leadingIcon="plus"`. 여러 개면 `gap-1.5`로 제목 뒤에 이어 붙인다.
   - 목업 원문이 우측 정렬(`.sacts` 등)이어도 이 규칙이 우선한다. 모달 **푸터**(닫기·저장)는 이 규칙 대상이 아니다 — 기존대로 `DialogFooter` 우측.
   - 적용처: `registry_ledger_modals.tsx` `Section`(등록원부 입력/수정·조합원·전문인력 모달), 소제목 선례 `subfund_form_modal.tsx`(업무집행조합원 `행 추가`)·등록원부 `약력`. 가드 테스트: `registry_ledger_modals.test.ts` "Section — 섹션 버튼은 제목 바로 옆 좌측".
   - ⚠ 미정합 잔존(별건): `gp_quant_indicator_modal.tsx`(표 아래 우측 행추가·행삭제)·`program_help_modal.tsx`(섹션 아래 좌측 `action`). 손댈 때 이 규칙으로 맞춘다.
...
> · **radio** = 이름으로 묶인 배타 그룹 — 하나를 고르면 나머지가 풀린다(개인/법인, 신주/구주, Y/N/해당없음, 검색 결과 1건 선택). select↔radio 는 스키마 선언대로 그린다 — 옵션 개수로 자동 치환하지 않는다(2026-09-22 개수 규칙 PR #236 은 09-23 사용자 원복).
