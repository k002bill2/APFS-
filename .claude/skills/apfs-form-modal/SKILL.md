...
5. **모달 기본 폰트 13.5px(2026-09-15 사용자 결정 — 종전 14px에서 하향).** 두 곳이 짝이다: ① `ui/dialog.tsx` `DialogContent`의 `text-[13.5px]`(모달 본문 상속 기본값) ② `renderers.tsx` `base`의 `fontSize: 13.5`(폼 컨트롤·radio 라벨·switch 상태 텍스트). 16px 아님(→[[responsive-ui]]). 색·테두리·배경은 토큰만(→[[color-tokens]]).
   ...
   - **입력과 한 줄에 놓이는 동거 버튼은 `CONTROL_BTN`(2026-09-21, PR #219).** 프로그램 검색·중복확인·해제·주소 검색·파일 선택처럼 컨트롤 옆에 붙는 `UI.Button size="sm"`은 `line-height:normal`이라 **실측 29px**로 34px 입력보다 5px 낮다. `renderers.tsx`의 **`CONTROL_BTN`**(CONTROL_BOX에서 height/minHeight/lineHeight 파생, 세로 패딩 0)을 `style={CONTROL_BTN}`으로 스프레드한다 — `UI.Button`은 `className`을 안 받으므로 `style`이 유일한 통로. ⚠ `style={{ height: 34 }}`·`CONTROL_BOX.height` 같은 부분 복사 금지(PR #219에서 3가지 표기를 하나로 모았다). ⚠ **전역 `sm` 규격(29px)은 그대로다** — 툴바·푸터·selbar의 sm 버튼은 컨트롤과 나란히 놓이지 않으므로 손대지 않는다(툴바 버튼 높이 규약 →[[apfs-stage-workflow]]). 검증: 모달 열고 `getBoundingClientRect().height`가 입력과 버튼 모두 34, `top` 동일.
...
- 규칙: ① Radix `Dialog` `max-w-[880px] max-h-[88vh]` + `onInteractOutside preventDefault`(RowFormModal과 동일) ② `<fieldset>/<legend>` 섹션, 본문은 `grid grid-cols-1 sm:grid-cols-2 gap-x-5`(wide 규격 동일) ③ **개별 컨트롤은 `SchemaField`(schemas/renderers.tsx) 재사용** — ad-hoc `FieldSpec`을 만들어 넘기면 14px·DatePicker·토큰이 자동(라벨 래퍼도 RowFormModal `Field` 규격 복제) ④ 반복행은 로컬 배열 state + `IconBtn icon="trash"` 행삭제 + `Button leadingIcon="plus"` 행추가 ⑤ 첨부는 hidden `<input type=file>` 1개를 슬롯별로 재사용(파일명만 보관, 백엔드 없음). **파일이 실린 슬롯 셀은 `ui/attachment.tsx`의 `Attachment` 카드**(확장자 아이콘+파일명+교체/삭제)로 렌더하되 단일 카드도 `AttachmentGroup`(role=list)로 감싼다(고아 listitem 방지 · web-a11y), 빈 슬롯은 `Button leadingIcon="upload" style={CONTROL_BTN}` [파일 선택](34px 정합, 위 규칙 5) — 드롭존(DocumentsField)과 같은 카드 프리미티브를 공유해 파일 표시를 단일화(2026-09-09) ⑥ 저장은 `onSave(patch: Partial<Row>)` — 문자열 폼값→`number|null`·`'YYYY-MM-DD'` 변환은 모달이 책임.
