...
## 구성 규약
1. **골격**: Radix `Dialog` `max-w-[880px] max-h-[88vh]`, 헤더 = **제목+대상명을 한 `<div className="flex flex-1 items-baseline gap-2.5 min-w-0 pr-8">` 래퍼로 묶어 좌측에 나란히** — 제목 `DialogTitle`(`shrink-0`), 대상명 `DialogDescription`(`truncate min-w-0`). ⚠ `DialogHeader`가 `justify-between`이라 **안 묶으면 대상명이 헤더 우측 끝으로 밀린다**(제목 옆이 아님). 본문 `overflow-y-auto p-[46px]`, 푸터 = `재무제표 상세`(primary) · `엑셀`(outline) · `닫기`. 섹션은 `<section><h3>제목<span 단위주석/></h3>` (`Section` 헬퍼). **본문이 `p-[46px]`라 `DialogHeader`·`DialogFooter`에도 `className="px-[46px]"`를 얹어 제목·버튼을 본문 좌우 기준선에 맞춘다**(중첩 `FsDetailModal` 포함, 2026-09-11 PR #132) — 정본 규약은 [[apfs-form-modal]] "모달 크롬 공통 규약" 5(공용 `ui/dialog.tsx` 기본 18px는 불변).
2. **금액 단위 토글**: 본문 최상단 우측 `SegTabs 원|백만원|억원`. 값 변환은 `money(won, unit)` 한 곳(원=`fmt`, 백만/억=소수 2자리, null→`-`). 중첩 팝업은 부모의 unit을 초기값으로 받되 자체 토글 보유.
...
8. **타이포(폰트 크기)**: 일반 페이지 본문(14px) 기준으로 맞춘다. 위계 = **모달 제목 `text-xl`(20) > 섹션 제목 `text-lg`(18)·`font-bold` > 본문 14**. 모달 헤더·본문 섹션 헤더의 정본 규약은 [[apfs-form-modal]] "모달 크롬 공통 규약"을 따른다 — 여기서 중복 정의하지 않는다. 요지: 모달 제목은 `DialogTitle` 공용 기본이 `text-xl`(**2026-09-09 사용자 결정으로 `ui/dialog.tsx` 기본값을 `text-base`→`text-xl`로 올려 전 모달 통일. 이전 '소비처 override·공용 불변' 규칙 폐기**) — 소비처에서 크기·굵기 재지정 금지. 개요 값(dd) 14 / 라벨(dt) 13, 재무제표 상세 표 14 / 재무요약 2단표 13, 대상명(`DialogDescription`) 13 · 단위주석·`금액 단위` 라벨 12.5. ⚠ **`preflight:false` + body base 폰트 없음** → bare heading은 UA 기본(h3≈18.7px)으로 튀므로, 섹션 제목에 인라인 `style={{fontSize}}` magic number 쓰지 말고 Tailwind `text-*` 유틸(`text-lg`)로 고정한다.
