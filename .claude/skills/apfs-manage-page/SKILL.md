...
| 검색박스(인라인 N개 필터) | **주 필터 1개=툴바 `FilterChip`**(예: 심사단계, 전체 칩은 `심사단계: 전체` → [[apfs-grid]]) + 나머지=**상세필터 드로어(Sheet)**, 검색어는 **기본 OFF opt-in**(필요한 페이지만 `searchable`/`SEARCHABLE`로 켜고, 켜질 때만 최상단 — 자동 추가 금지), 컬럼 미연동 필터는 `· 데이터 연동 후 적용` 캡션 | [[apfs-detail-filter]] |
...
| 파일 업로드(목업 `.uploadbox`·파일명 드롭존·업로드 폼) | **툴바 독립 버튼 `[업로드]`**(`variant="outline" size="sm" leadingIcon="upload"`, 등록 버튼 오른쪽) → **공용 `UploadModal`**(`trust_upload.tsx` — 파일명 드롭존 + [닫기]/[확인], `emptyMsg`·`doneMsg`는 원문 토스트 문구). **드롭존을 카드 본문(그리드 위 섹션)에 펼쳐 두지 않는다**(2026-09-24 사용자 결정 — 정본 예시 `계좌정보 관리` `trust_upload_forms.tsx`). 모달을 새로 만들지 말고 이 부품을 재사용. 가드 테스트 `trust_pages_13.test.ts` '업로드 = 툴바 [업로드]' | 드롭존 본체=[[apfs-form-modal]] `file` 규약(`UploadDropzone`→`DocumentsField`) |
