...
| 행 라디오 단일선택 | `rowSelection={{mode:'singleRow',checkboxes:true,enableClickSelection:true}}` + `selectionColumnDef={{pinned:'left'}}` + `getRowId` 안정 id. ⚠ **선택이 액션(선택삭제·단계전이·행 편집)을 만들 때만** — 조회 전용이면 `checkboxes:false` + `selectionColumnDef` 생략 | [[apfs-aggrid]] |
