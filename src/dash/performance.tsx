  ...
  return h(React.Fragment, null, h("div", { className: "max-w-[1320px] mx-auto", style: { animation: "dashFade .35s var(--ease) both" } },
    ...
    h("section", { className: "rounded-card-lg border border-border bg-card shadow-sm overflow-hidden mb-4" },
      ...
      h("div", { className: "flex items-center justify-between gap-4 flex-wrap px-5 sm:px-6 pt-5 pb-4" },
        h("div", { className: "flex items-center gap-2.5" },
          h("h2", { className: "text-[19px] font-bold tracking-[-.02em]" }, "투자포트폴리오"),
          h("span", { className: "inline-flex items-center gap-1 text-caption text-[12.5px] font-semibold" },
            h(Icon, { name: "check-circle", size: 14, style: { color: "var(--warning)" } }), "120")),
        h("div", { className: "flex items-center gap-2.5" },
          h(StatPill, { icon: "trending", label: "일일 수익률", value: "+12.4%", tone: "var(--success)" }),
          h(StatPill, { icon: "wallet", label: "순자산", value: "₩4.2조" }))),
      ...
      h("div", { className: "flex items-center gap-2 flex-wrap px-5 sm:px-6 py-3 border-t border-border" },
        ...
        chips.length
          ? chips.map((c) => h(FilterChip, { key: c.key, active: true, onClick: () => removeChip(c.key) }, h("span", null, c.label), h(Icon, { name: "x", size: 13 })))
              ...
              h("td", { className: "px-4 pl-5 sm:pl-6 py-3.5" },
                h("div", { className: "flex items-center gap-3" },
                  ...
                  h("div", { className: "min-w-0" },
                    h("div", { className: "text-[14.5px] font-bold leading-tight", style: { color: "var(--foreground)" } }, r.name),
                    h("div", { className: "t-caption mt-0.5" }, r.meta)))),
              /* 가치 */
              h("td", { className: "px-4 py-3.5 text-right tabular text-[14.5px] font-semibold whitespace-nowrap", style: { color: "var(--foreground)" } }, r.value),
              /* 변동폭 */
              h("td", { className: "px-4 py-3.5 text-right tabular text-[14px] font-bold whitespace-nowrap", style: { color: changeColor(r.change) } }, fmtChange(r.change)),
              /* 리스크 등급 */
      ...
      h("div", { className: "flex items-center justify-between gap-4 flex-wrap px-5 sm:px-6 py-4 border-t border-border" },
        h("span", { className: "t-caption" }, "총 1,208개 중 ", h("b", { style: { color: "var(--foreground)" } }, rows.length + "개"), " 항목 표시 중"),
    ...
    h("div", { className: "grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-4" },
      ...
      h("div", { className: "rounded-card-lg border border-border p-6", style: { background: "color-mix(in srgb,var(--muted) 50%,var(--card))" } },
        h("h3", { className: "text-[17px] font-bold mb-3" }, "분기별 전망"),
        h("p", { className: "t-body text-[13.5px] leading-relaxed", style: { color: "var(--muted-foreground)", maxWidth: 540 } },
          "재무 모델링에 따르면 농식품 정책펀드 포트폴리오는 강세 추세를 보일 것으로 예측됩니다. 다음 회계연도에는 스마트팜·푸드테크 등 신성장 분야로의 다변화를 권장합니다."),
        h("div", { className: "flex items-end gap-10 mt-6" },
          h("div", null,
            h("div", { className: "t-caption mb-1" }, "신뢰 지수"),
            h("div", { className: "text-[30px] font-extrabold tabular", style: { color: "var(--accent)" } }, "88%")),
          h("div", null,
            h("div", { className: "t-caption mb-1" }, "변동성 지수"),
            h("div", { className: "text-[30px] font-extrabold", style: { color: "var(--success)" } }, "낮음")))),

      /* 자본 준비금 (단색) */
      h("div", { className: "rounded-card-lg p-6 text-white relative overflow-hidden", style: { background: "#439E00", boxShadow: "var(--shadow-md)" } },
        h("div", { className: "relative" },
          h("h3", { className: "text-[17px] font-bold mb-1.5" }, "자본 준비금"),
          h("p", { className: "text-[13px] mb-1", style: { opacity: .85 } }, "출자 가능 미집행 자금 현황입니다."),
          h("div", { className: "text-[34px] font-extrabold tabular mb-5 leading-tight" }, "₩1,402,990", h("span", { className: "text-[16px] font-semibold ml-1", style: { opacity: .8 } }, "백만")),
          h("button", {
