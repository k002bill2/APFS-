...
const columnDefs: (ColDef<SubFundRow> | ColGroupDef<SubFundRow>)[] = [
  ...
  { field: 'stg', headerName: '심사단계', width: 96, pinned: 'left', cellStyle: flexMid, sortable: true,
    cellRenderer: (p: any) => (p.node.rowPinned ? null : <StatusBadge tone={STAGE_TONE[p.value as Stage]} label={p.value} size="sm" />) },
      ...
      toolbarLeft={selected ? (
        ...
        <>
          <span className="inline-flex items-center gap-1.5 font-semibold" style={{ fontSize: 13 }}>
            <StatusBadge tone={STAGE_TONE[selected.stg]} label={selected.stg} size="sm" />
          ...
          {filteredRows.map((r) => (
            <button key={r.id} type="button" onClick={() => setSelId(r.id)} aria-pressed={selId === r.id}
              ...
              <div className="flex items-center gap-2.5">
                ...
                <StatusBadge tone={STAGE_TONE[r.stg]} label={r.stg} size="sm" />
              </div>
