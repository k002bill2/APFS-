/* 운용사 주주변동관리 — 관리형 리스트 페이지 (조기경보 > 조기경보 > 운용사 주주변동관리).
   ...
   - `구분` 셀 → `StatusBadge size="lg"`. 등록=info · 해제=muted(형제 화면과 동일 매핑,
...
const kindCell = (p: { value: ShareholderKind }) => <StatusBadge tone={KIND_TONE[p.value]} label={p.value} size="lg" />;
/* 날짜 셀 — 빈 값은 '-' */
...
export function ShareholderManage({ onNav }: { onNav?: (r: string) => void }) {
  ...
  const selHasReleased = useMemo(() => rows.some((r) => r.g === '해제' && selIds.includes(r.id)), [rows, selIds]);
  ...
  const onSelectionChanged = useCallback((e: SelectionChangedEvent<ShareholderRow>) => { setSelIds(e.api.getSelectedRows().map((r) => r.id)); }, []);
  ...
  const openRelease = () => {
    ...
    if (sel.some((r) => r.g === '해제')) { toast('이미 해제된 행은 해제등록할 수 없습니다'); return; }
  ...
  const commitRelease = ({ rd }: { rd: string }) => {
    ...
    const ids = new Set(modal.ids);
    setRows((prev) => prev.map((r) => (ids.has(r.id) && r.g === '등록' ? { ...r, g: '해제' as ShareholderKind, rd } : r)));
    setModal(null);
  };
