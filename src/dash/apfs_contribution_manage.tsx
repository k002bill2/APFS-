/* (농금원)출자배분관리 — 조회형 리스트 페이지 (투자자산관리 > 자펀드 관리 > (농금원)출자배분관리).
...
export function ApfsContributionManage({ onNav }: { onNav?: (r: string) => void }) {
  ...
  const exportExcel = () => {
    ...
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, '농금원출자배분관리');
    XLSX.writeFile(wb, `(농금원)출자배분관리_${unit}.xlsx`);
  ...
  return (
    <GridFrame
      crumbs={['홈', '투자자산관리', '자펀드 관리', '(농금원)출자배분관리']}
      title="(농금원)출자배분관리"
      ...
      <Sheet open={filterOpen} onOpenChange={setFilterOpen}>
        <SheetContent side="right" hideClose className="w-[408px] max-w-[92vw]">
          <SheetHeader>
            ...
            <SheetDescription className="sr-only">(농금원)출자배분관리 목록을 거르는 상세 필터</SheetDescription>
