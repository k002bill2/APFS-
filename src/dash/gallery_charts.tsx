...
function ColumnTrack({ data, height = 160, highlight = null, accent = "var(--primary)", soft = false }: { data: any[]; height?: number; highlight?: number | null; accent?: string; soft?: boolean }) {
  ...
  return (
    <div ref={ref} className="relative w-full" style={{ height }}>
      ...
      }}>{String(data[hi].label || data[hi].value)}</div>}
    </div>
  );
}
      ...
      {W > 0 && <svg width={W} height={height} role="img" aria-label={`${data.length}개 카테고리 비중 파이 차트`}>
        {data.map((d, i) => {
          ...
          return <g key={i}>
            ...
            {frac > 0.06 && <text x={lx} y={ly+4} textAnchor="middle" style={{ fontSize: 12, fontWeight: 800, fill: "var(--on-chart-fill)" }}>{String(Math.round(frac*100))+"%"}</text>}
          </g>;
        })}
      </svg>}
    </div>
