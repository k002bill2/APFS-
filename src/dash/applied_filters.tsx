/* 적용된 필터 줄 — GridFrame 툴바 아래 둘째 줄의 SSOT (2026-09-24 사용자 결정, 전 화면 공통).
   ...
   - 값이 있는 항목이 하나라도 있을 때만 줄이 나타난다. 앞에 `적용된 필터` 캡션 + role="group".
   ...
   - 줄은 감긴다(flex-wrap) — 해제 대상이라 스크롤 안에 숨기면 안 된다.
...
/** GridFrame 가 툴바 바로 아래에 렌더한다. 페이지는 직접 쓰지 않고 `appliedFilters` prop 으로 넘긴다. */
export function AppliedFiltersRow({ items }: { items: readonly AppliedFilter[] }) {
  ...
  return (
    <div role="group" aria-label="적용된 필터" className="flex items-center flex-wrap gap-2"
      ...
      <span className="text-caption shrink-0" style={{ fontSize: 12.5 }}>적용된 필터</span>
      {on.map((f) => <Chip key={f.label} f={f} />)}
      {clearable.length >= 2 && (
        <button type="button" onClick={() => clearable.forEach((f) => f.onClear?.())}
          className="border-0 bg-transparent cursor-pointer text-muted-foreground hover:text-foreground underline-offset-2 hover:underline"
          style={{ fontSize: 12.5, padding: '4px 6px', fontFamily: 'inherit' }}>
          전체 해제
        </button>
      )}
    </div>
