...
function AuditDetailModal({ row, onClose }: { row: AuditRow; onClose: () => void }) {
  const items: [string, ReactNode][] = [
    ['일시', row.ts], ['행위자', row.actor], ['유형', <StatusBadge tone={KIND_TONE[row.kind]} label={row.kind} size="md" />],
    ['행위', row.action], ['대상', row.target], ['IP', row.ip], ['결과', <StatusBadge tone={RESULT_TONE[row.result]} label={row.result} size="md" />],
  ];
