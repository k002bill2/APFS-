...
2. ~~`program_manage.tsx` 부재 — 신규.~~ **철회(2026-09-14 사용자 지시 "되돌려")** → **재도입(2026-09-15, 머지 `c8e3c39`)** — 철회 근거는 "탭 바의 첫 탭"이라는 것뿐이었는데, 이후 사용자 이미지 IA 브리프(`dev/active/admin-pages-complete/BRIEF.md`)가 **탭이 아닌 LNB 리프**로 재지시하며 원본 목업 `S0_108_프로그램관리.html`을 새 근거로 제시했다(사용자 재확인 2026-09-15 "프로그램관리 유지해줘"). 현재 상태: 페이지 전면 보강(등록·수정·삭제 + 도움말 편집 + 연결 프로그램 삭제 게이트)·`programCatalog`/`ProgramEntry` 복원·테스트 복원. 참고 이력(철회 전 구 버전): 데이터는 `admin_menu_tree.programCatalog`(LNB 정본 리프 = 목업 S0_105 `PROGRAMS`)만 사용. 목록·검색(프로그램ID·프로그램명 부분일치 = 목업 프로그램 검색 팝업)·사용여부 칩·엑셀·페이지네이션 20. **CRUD 없음**(근거 없음).
...
4. ~~`data.ts`/`app.tsx` program-manage 누락~~ **철회(2026-09-14)** → **재도입(2026-09-15, 머지 `c8e3c39`)** — 리프 `프로그램관리`(path `program-manage`)가 관리자 IA 3중분류(`사용자·권한 관리`/`시스템 관리`/`감사·기록`, 리프 8개) 중 `시스템 관리` 첫 항목으로 복귀했고 app.tsx import·route 분기·별칭(`프로그램 관리`·`프로그램관리`)도 살아 있다. "구조표 정본 리프 141개 복귀"는 이 이미지 IA 재편으로 대체됐다(가드: `admin_ia.test.ts`). 참고 이력(철회 전 구 버전): 리프 `프로그램 관리`(path `program-manage`)를 `시스템 관리` 첫 항목으로 추가(구조표에 없던 리프 — 이 브리프의 사용자 지시가 근거), 순서 프로그램→공통코드→메뉴→도움말. app.tsx import·route 분기·별칭(`프로그램 관리`·`프로그램관리`).

결정·가정:
- ~~`프로그램 관리` 리프 추가로 프로그램ID 일련이 1 밀렸다. 권한 매트릭스 행 143 = 리프 142 + 1.~~ **철회 후 원복(2026-09-14) → 관리자 IA 재편으로 재산정(2026-09-15)** — 구체 개수를 다시 적지 않는다: 카운트는 `APFS_DATA.ALLMENU`에서 파생이고 테스트에 하드코딩이 없어 IA가 바뀌면 자동 추종한다.
