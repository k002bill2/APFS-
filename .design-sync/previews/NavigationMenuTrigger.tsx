...
/* NavigationMenuTrigger — 레일의 아이콘 버튼. DS 는 `cursor-pointer outline-none` 만 주고
   크기/표면/활성색은 소비처가 className·style 로 지정한다(hover 는 Radix 기본, 열림 상태는 data-[state=open]).
   호버뿐 아니라 **포커스·Enter 로도** 플라이아웃이 열리는 키보드 진입 경로를 제공한다. */
...
const cap: React.CSSProperties = { fontSize: 11.5, color: 'var(--caption)' };
...
export function TriggerStates() {
  ...
  return (
    <div style={{ position: 'relative', height: 300, display: 'flex' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, padding: '14px 0 0 296px', display: 'flex', flexDirection: 'column', gap: 26 }}>
        <div style={cap}>1번째 — 활성(aria-current=page): primary 배경</div>
        <div style={cap}>2번째 — 기본 상태</div>
        <div style={cap}>3번째 — 열림(data-state=open): 플라이아웃 표시, 미확인 빨간 점</div>
      </div>
    </div>
  );
}
