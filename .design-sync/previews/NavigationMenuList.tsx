...
/* NavigationMenuList — 레일의 <ul>. preflight:false 환경이라 `list-none m-0 p-0` 으로 UA 마커/패딩을 직접 끈다.
   `flex flex-col` 기본(세로 레일). 간격은 소비처가 className(gap-1, py-3 px-2)으로 준다. */
  ...
  return (
    <div style={{ position: 'relative', height: 300, display: 'flex' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, padding: '14px 18px 14px 296px' }}>
        <div style={{ fontSize: 11.5, color: 'var(--caption)' }}>세로 레일 List — 대분류 5개가 list-none 으로 마커 없이 적층</div>
      </div>
    </div>
  );
}
