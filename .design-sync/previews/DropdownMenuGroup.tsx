...
/* 그룹 — 라벨이 붙는 항목 묶음(role=group). 구분선과 함께 메뉴를 구역으로 나눈다. */
export function GroupedActions() {
  return (
    <div style={stage}>
      <DropdownMenu open onOpenChange={() => {}}>
        <DropdownMenuTrigger className={TRIGGER_OUTLINE}>자펀드 작업</DropdownMenuTrigger>
        <DropdownMenuContent align="start" style={{ width: 240 }} onOpenAutoFocus={noAutoFocus}>
          <DropdownMenuGroup>
            <DropdownMenuLabel>조회</DropdownMenuLabel>
            <DropdownMenuItem>자펀드 명세</DropdownMenuItem>
            <DropdownMenuItem>출자 이행 내역</DropdownMenuItem>
          </DropdownMenuGroup>
