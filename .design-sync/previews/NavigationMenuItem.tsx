...
/* NavigationMenuItem — 레일 한 칸. `value` 를 주면 제어형 NavigationMenu 의 열림 대상이 된다.
   자식 메뉴가 있으면 Trigger + Content 쌍을, 없으면(대시보드 등) NavigationMenuLink 단독을 담는다. */
...
export function TriggerAndLinkItems() {
  const open = 'fund';
  return (
    <div style={{ position: 'relative', height: 320, display: 'flex' }}>
      <nav aria-label="주 메뉴" className="bg-card" style={{ width: 64, height: 320, borderRight: '1px solid var(--border)', position: 'relative', zIndex: 48 }}>
          ...
          <NavigationMenuList className="gap-1 py-3 px-2">
            ...
            {MENU.slice(0, 4).map((m) => (
              <NavigationMenuItem key={m.id} value={m.id}>
                ...
                <NavigationMenuContent style={{ left: 72, top: 0, width: 264, maxHeight: 320 }}>
      ...
      <div style={{ flex: 1, minWidth: 0, padding: '14px 18px 14px 296px' }}>
        <div style={{ fontSize: 11.5, color: 'var(--caption)', marginBottom: 6 }}>맨 위 = 자식 없는 Item(Link 단독) · 나머지 = Trigger+Content 쌍</div>
        <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--foreground)' }}>모태펀드 관리</div>
      </div>
    </div>
  );
}
