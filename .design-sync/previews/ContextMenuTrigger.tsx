...
/* 트리거 — 우클릭을 받는 영역(여기서는 그리드 전체). Radix 가 커서 좌표에 메뉴를 앵커한다. */
export function GridAreaTrigger() {
  const ref = React.useRef<HTMLElement | null>(null);
  useOpenAt(ref, 150, 118);
  return (
    <div style={stage}>
      <div style={{ fontSize: 11.5, color: 'var(--caption)', marginBottom: 8 }}>표의 아무 행에서나 우클릭 → 행 액션 메뉴</div>
      <ContextMenu>
        <ContextMenuTrigger ref={ref as any} style={{ display: 'block' }}>
          <RowsFixture />
        </ContextMenuTrigger>
        <ContextMenuContent style={{ width: 196 }} onOpenAutoFocus={noAutoFocus}>
