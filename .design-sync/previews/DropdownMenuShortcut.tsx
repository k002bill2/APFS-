...
/* 단축키 힌트 — 항목 우측(ml-auto) caption·넓은 자간. aria-hidden(실제 바인딩은 use-hotkey 가 등록). */
export function ShortcutHints() {
  return (
    <div style={stage}>
      <DropdownMenu open onOpenChange={() => {}}>
        <DropdownMenuTrigger className={TRIGGER_OUTLINE}>바로가기</DropdownMenuTrigger>
        <DropdownMenuContent align="start" style={{ width: 248 }} onOpenAutoFocus={noAutoFocus}>
          <DropdownMenuLabel>목록 단축키</DropdownMenuLabel>
          <DropdownMenuItem>등록<DropdownMenuShortcut>⌘⏎</DropdownMenuShortcut></DropdownMenuItem>
          ...
          <DropdownMenuItem>메모<DropdownMenuShortcut>⌥M</DropdownMenuShortcut></DropdownMenuItem>
          <DropdownMenuItem>일정<DropdownMenuShortcut>⌥S</DropdownMenuShortcut></DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
  );
