...
/* ContextMenu — AG Grid 행 우클릭 메뉴(Community 대체)의 DS 표면. dropdown-menu 와 동일 토큰.
   ⚠ `open` 제어 prop 은 있으나 사용자 상호작용 전에 true 면 Radix 가 앵커를 못 잡아 뷰포트
     좌상단(0,0)에 붙는다(콘솔 경고까지 낸다). 그래서 프리뷰는 마운트 직후 트리거에 실제
     contextmenu 이벤트를 디스패치해 커서 좌표에서 열리게 한다 — 제품과 같은 경로.
...
/* 대표 스토리 — 행 우클릭 메뉴(라벨·구분선·삭제 danger). */
  ...
  return (
    <div style={stage}>
      <ContextMenu>
        ...
        <ContextMenuContent style={{ width: 200 }} onOpenAutoFocus={noAutoFocus}>
          <ContextMenuLabel>농식품 청년창업 투자조합 2호</ContextMenuLabel>
          <ContextMenuItem>상세 보기</ContextMenuItem>
          <ContextMenuItem>수정</ContextMenuItem>
          <ContextMenuItem>즐겨찾기 추가</ContextMenuItem>
          <ContextMenuSeparator />
          <ContextMenuItem>엑셀 내보내기</ContextMenuItem>
          <ContextMenuSeparator />
