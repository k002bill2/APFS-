...
> 작성: 멀티에이전트 워크플로우(resume로 완주) — Research/Map/**적대적 Verify 8건/리포트 모두 완료**
> 근거: AG Grid 공식문서 1차 출처(ag-grid.com·license-pricing·npm) 직접 판독 + 적대적 반증 + 직전 IBSheet 리포트 비교
> 독자: APFS 프론트엔드 결정권자 | 작성일 2026-06-22 | 기준 버전: v33+(최신 v35.3.1)

> ✅ **방법론(검증 완료):** AG Grid 능력 판정은 Research 에이전트 3종이 공식문서를 브라우저로 직접 읽어 확보한 **1차 출처**(에디션은 `(e)` 마커·license-pricing Feature Breakdown·npm `license` 필드로 교차확인)에 더해, **회의적 검토관 8명이 핵심 주장을 공식문서로 반증 시도(적대적 검증)** 한 결과로 확정됐다. **AG Grid 자체 판정은 refuted 0건**(confirmed 4 / partial 4) — 'REFUTED' 태그는 모두 IBSheet 측 실패점이다. 전체 검증 결과는 **부록 D** 참조. 표기 `[1차출처]`=공식문서 직접 확인, `[검증]`=적대적 재검증 통과, `[미검증]`=확인 불가.

> ⚠️ **백분율 주의:** 본 리포트의 분모는 GridFrame 양식 **36개 기능점**(직전 IBSheet 리포트는 46건) — 분모가 달라 백분율 직접 비교 금지. 핵심 결론(마찰점 해소 여부)으로 비교할 것.
...
| 기능 | AG Grid지원 | 에디션 | 평가 | 공수 | 리스크 | vs IB | 비고 |
|---|---|---|---|---|---|---|---|
| **X1 마스킹 mn()/MT 전역 토글** | custom | 무료 | **workaround** | medium | medium | **▲(약간)** | **[검증·partial]** native 전역토글 없음(IBSheet와 동일). 텍스트=`valueFormatter`가 `mn()` 참조, 배지/스파크라인=렌더러 내부 분기. **블로커: 현 `mask.tsx`의 `_on`이 로드시 `const`(16행)라 런타임 토글 불가** → 3작업 전제: ⓐ`_on` 가변화, ⓑ토글 후 `refreshCells({force:true})`(마스크는 행 값 불변 → force 없으면 change-detection이 전부 skip), ⓒmemo 렌더러가 토글 신호 재독취. 표준 Community API만으로 가능하나 자동 아님 |
...
## 부록 C. 잔여 미검증 항목 (PoC 실측 후 갱신)
- **번들 크기 정량** — ✅ **PoC 실측 완료(부록 E)**: AllCommunityModule = **+1,157 kB min / +326 kB gzip**(gzip 2.2배). worst-case 상한 — 특정 모듈만 import 시 트리쉐이킹으로 축소 가능.
- `var(--token)` ↔ theme 파라미터 **전 크롬 매핑 완전성** — ✅ **PoC 실측 완료(부록 E)**: 핵심 4개 파라미터만 바인딩 → 헤더 배경·테두리·행 색 등 **파생 크롬이 라이트/다크 모두 자동추종, drift 없음.**
- **full-width 행(fullWidthCellRenderer)의 행선택 무손실** — 여전히 `[미검증]`(매트릭스 PoC 범위 밖 — generic_list/모바일 2차 PoC 몫)
- **M2 합계행 "앱이 전 화면 합계 사전계산" 전제의 보편성** — `[검증·partial]`(화면별 점검 필요 — 단 PoC의 단일 총계는 `pinnedBottomRowData`로 정상 동작)
- pinned row 스크린리더 한계가 공공기관 접근성 기준 충족 여부 — `[1차출처·영향 미평가]`

## 부록 D. 적대적 검증 결과 (8건 — refuted 0, confirmed 4 / partial 4)

| # | 검증한 주장 | 판정 | 핵심 근거 / 정정 | 출처 |
|---|---|---|---|---|
| D-1 | **X2 테마**: Tweaks 런타임 CSS변수 편집이 Theming API로 라이브 전파되는가 | ✅ **confirmed** | `withParams`는 var() 리터럴을 **가공 없이** CSS 커스텀 프로퍼티로 주입 → 브라우저가 paint시 resolve → `--primary` mutate 시 **theme 재생성 없이 자동추종**. `data-ag-theme-mode`로 다크 동기화. **원안 risk:high/workaround 폐기 → gain 확정.** 전제 v33+. 전부 Community | theming-parameters / theming-css |
| D-2 | **L10 스파크라인**: native는 Enterprise지만 자체 MiniBars를 Community 셀렌더러로 넣으면 무료 | ✅ **confirmed** | native `agSparklineCellRenderer`=Enterprise(SparklinesModule). 임의 SVG를 `colDef.cellRenderer`로=Community/MIT, 버전게이트 없음. **Enterprise 불필요** | sparklines-installation |
| D-3 | **L11/L12 필터**: Community External Filter로 복합식(AND/태그OR/부분·정확) 단일 콜백 이식, Set/Advanced Filter 불요 | ✅ **confirmed** | `ExternalFilterModule`=Community, `doesExternalFilterPass`는 임의 행 술어. Set/Multi/Advanced=Enterprise이나 우리 드로어가 UI 제공 → 불요. **단 CSRM 전용·UI 미제공** | iModule.ts |
| D-4 | **멀티컬럼 정렬**이 Community(마케팅 묶음이 Enterprise처럼 보일 뿐) | ✅ **confirmed** | Row Sorting 문서·npm MIT·소스(community-modules/core)·비교표 4개 교차일치. Enterprise가 파는 건 'advanced grouping/sorting'이지 멀티정렬 자체 아님 | row-sorting |
| D-5 | **M2 합계행** = `pinnedBottomRowData`(Community), Enterprise aggregation 불요 | 🟡 **partial** | 라이선스 경계 confirmed. **단 "앱이 전 화면 합계 사전계산" 전제는 문서로 보편성 인증 불가** → 화면별 점검 | license-pricing |
| D-6 | **라이선스**: Community MIT·키불요·도메인무바인딩, baseline이 Enterprise 안 끌어들임 | 🟡 **partial** | 라이선스 confirmed(Vercel preview 그냥 동작). **단 "baseline 100% Community"는 AG Grid 미도입이라 vacuously true** → 도입 PR에서 import 감사 필요 | license-pricing |
| D-7 | **X1 마스킹**: mn/MT 재사용 + `refreshCells({force:true})`로 런타임 전역 가림 동작 | 🟡 **partial** | 셀렌더러·valueFormatter·refreshCells 전부 Community. `force:true`가 핵심(마스크는 행 값 불변). **단 `mask.tsx _on`이 const라 런타임 토글 불가 → 가변화+force호출+memo재독취 3작업 전제** | view-refresh |
| D-8 | **X3 반응형**: 모바일 카드재배치 native 미지원(loss), 우회 시 정렬/선택/가상화 무손실 | 🟡 **partial** | native 미지원 confirmed(GitHub #3137/#3556). 우회 시 **데이터레벨 정렬·세로 가상화는 보존되나, 카드 측 정렬/선택 조작 UI는 직접 재구현**, full-width 행선택 무손실은 미검증. 우회비용 high. IBSheet도 불가 → **tie** | full-width-rows |

> **검증 요지:** AG Grid 자체엔 refuted가 없다. confirmed 4건은 도입의 핵심 이득(테마·스파크라인·필터·정렬)을 굳혔고, partial 4건은 "되지만 전제/배선이 있다"는 정직한 단서(합계행 전제·라이선스 감사·마스킹 리팩터·반응형 우회)다.

## 부록 E. PoC 실측 결과 (2026-06-23, AG Grid Community v35.3.1)

`asset_funding`을 **frame+children 점진전략**으로 PoC 구현(`src/dash/asset_funding_aggrid.tsx`, 원본 무손상). 라우트 `asset-funding-aggrid`, 원본 페이지 헤더에 "AG Grid PoC" 버튼으로 왕복. 실제 dev 서버(localhost:5273)에서 라이트/다크 렌더·콘솔·정렬 확인.

| 검증 항목 | 결과 | 비고 |
|---|---|---|
| **빌드** | ✅ green | `vite build` exit 0, 3455 modules |
| **렌더(런타임)** | ✅ 정상 | blank grid 없음 — `ModuleRegistry.registerModules([AllCommunityModule])` 등록 효과 |
| **콘솔** | ✅ 클린 | AG Grid 에러·모듈 미등록·테마 충돌·라이선스 워터마크 **0건**(Vite HMR 메시지만) |
| **테마 자동추종(X2)** | ✅ **confirmed 실증** | `themeQuartz.withParams`에 핵심 4개(`backgroundColor/foregroundColor/accentColor/borderColor`)만 `var(--token)` 바인딩 → **`data-ag-theme-mode` 없이 `.dark` 캐스케이드로 다크 자동전환**, 헤더 배경·테두리·행 색 등 **파생 크롬 drift 없음** |
| **2단 중첩헤더(M1)** | ✅ 정상 | 구분(고정열) \| 조성현황(6열 그룹) \| 출자현황(2열 그룹), `marryChildren` |
| **pinned 합계행(M2)** | ✅ 정상 | `pinnedBottomRowData`로 하단 고정, bold, **정렬 시에도 데이터에 섞이지 않고 고정 유지** |
| **정렬(Community)** | ✅ 정상 | 헤더 클릭 → 행 재정렬 + 정렬 화살표, 합계행 제외 |
| **마스킹(X1)** | ✅ valueFormatter 경로 동작 | `mn()`을 valueFormatter에서 호출 → 전 숫자 `000` 마스킹 표시(mask `_on` ON 상태 반영). **단 런타임 토글은 미검증**(`_on` const, D-7 전제) |
| **선택 액션(L1·L2)** | ✅ 정상 | `rowSelection`(v35 객체 API) 체크박스 33개 렌더 → 2건 선택 시 "2건 선택됨 / 선택 삭제 / 선택 해제" 툴바 전환, 삭제 동작. `onSelectionChanged`→React state |
| **kebab 메뉴(L16)** | ✅ 정상 | Radix DropdownMenu(generic_list MoreMenu 동형): 등록 / 내보내기 / 인쇄 — GridFrame toolbarRight 슬롯(순수 React) |
| **CSV 내보내기(X5)** | ✅ Community | `exportDataAsCsv()` 클라이언트 단독·서버 불요(IBSheet POI 서버모듈 대비 우위). Excel(.xlsx)만 Enterprise |
| **페이지네이션(L5)** | ✅ Community | `pagination`+`paginationPageSize=12`→16행 2페이지. `suppressPaginationPanel`로 AG Grid 페이저 숨기고 **커스텀 페이저(footerCenter)가 `paginationGoToPage()` 제어** — 엔진 AG Grid, 외관 토큰. 페이지당 12행만 렌더 확인 |
| **뷰토글 리스트↔카드(L6)** | ✅ **실증** | 푸터 SegTabs로 전환: **리스트=AG Grid 표 / 상세=React 카드, 단일 rowData 공유**(렌더러 이원화). IBSheet 카드뷰 unverified였던 지점을 AG Grid는 React 카드 병행으로 해소 |
| **푸터 슬롯(건수/페이저/뷰토글/아이콘)** | ✅ 정상 | "총 N개 중 M개 표시 중"+페이저+리스트/상세+다운로드/새창/더보기 — 전부 GridFrame footerLeft/Center/Right(순수 React) |
| **상세필터(L12, External Filter)** | ✅ **실증·Community** | Sheet 드로어(ui/sheet 재사용)→`isExternalFilterPresent`/`doesExternalFilterPass`+`onFilterChanged()`. "출자금액 ≥ 1000" 적용 시 16행→6행 정확 필터, 필터 칩+제거 동작. **L12(복합 외부필터=Community) 실측 확인** |
| **등록(CRUD add)** | ✅ 정상 | Dialog(ui/dialog 재사용)→입력→`setRows` prepend. 2026 추가 시 그리드 최상단 반영 + 활성 필터 동시 통과 |
| **번들 증분** | ⚠️ **+1,157 kB min / +326 kB gzip** | baseline 267 kB → 593 kB gzip(2.2배). AllCommunityModule worst-case — 특정 모듈만 import 시 축소 가능 |

**PoC 결론:** `frame+children` 전략은 **실제로 깨끗하게 동작**한다 — GridFrame(프레임)은 무변경, AG Grid가 `children`만 차지. **가장 큰 불확실성이었던 테마 자동추종(X2)이 실제 앱에서 라이트/다크 모두 drift 없이 확인**됐다. 추가로 **generic_list 동형 명령 툴바(행 선택→선택 삭제/해제 · 상세필터 · 새로고침 · kebab 등록/내보내기/인쇄)와 푸터(건수 · 페이지네이션 · 리스트↔카드 뷰토글 · 다운로드/새창/더보기)까지 AG Grid에 그대로 이식·실측 완료** — 선택은 `rowSelection`, 페이징은 `pagination`+커스텀 페이저, 뷰토글은 표(AG Grid)↔카드(React) 단일 데이터 공유(L6), 내보내기는 `exportDataAsCsv()`(Community), 메뉴는 Radix(toolbar/footer 슬롯=순수 React). 또한 **등록(Dialog→행 추가)과 상세필터(Sheet→AG Grid External Filter, L12)도 실연동·실측 완료** — 모달/드로어 UI는 ui/dialog·ui/sheet 프리미티브 재사용, 필터는 Community external filter로 16행→6행 정확 동작. 유일한 실질 비용은 **번들 +326 kB gzip**(worst-case). **미검증으로 남은 것**(generic_list 2차 PoC 몫): React 셀 렌더러 재사용(StatusBadge/MiniBars), **CRUD 수정/삭제 폼**(등록 add는 완료, edit/2단삭제확인은 미구현), **마스킹 런타임 토글**(`mask.tsx _on` 가변화 선행 필요), 모바일 카드 재배치(X3). ⚠️ asset_funding PoC가 green이라고 "AG Grid 전면 검증"으로 읽으면 안 된다 — 위 미검증 표면이 generic_list PoC를 필요로 한다.
