...
## 부록 C. 잔여 미검증 항목 (적대적 검증 후에도 문서로 확정 불가)
아래는 적대적 검증 8건을 거치고도 **공식문서만으로는 확정 불가** — 도입 PoC에서 실측/실험 필요:
- **번들 크기 정량**(v33 모듈 트리쉐이킹 + `AllCommunityModule` 범위 실측) — `[미검증]`
- **full-width 행(fullWidthCellRenderer)의 행선택 무손실** — 공식문서 직접 보증 미확인 `[미검증]`
- **M2 합계행 "앱이 전 화면 합계를 사전계산" 전제의 보편성** — 서버/그리드 집계가 필요한 화면이 숨어있지 않은지 화면별 점검 요 `[검증·partial]`
- `var(--token)` ↔ AG Grid theme 파라미터 **전 크롬 항목(헤더·교차행·필터행) 매핑 완전성** — 메커니즘은 검증됨(부록 D-1), 전 항목 1:1 매핑은 PoC 확인 `[추론]`
