...
# ─ ~/.claude/CLAUDE.md 및 ~/.claude/agents/{architect,worker,analyzer}.md 생성
# ─ 구버전 reviewer.md 제거(검증은 Codex로 대체)
...
backup_if_exists "${AGENTS_DIR}/architect.md"
cat > "${AGENTS_DIR}/architect.md" <<'ARCHMD'
---
name: architect
...
backup_if_exists "${AGENTS_DIR}/worker.md"
cat > "${AGENTS_DIR}/worker.md" <<'WORKERMD'
---
name: worker
...
backup_if_exists "${AGENTS_DIR}/analyzer.md"
cat > "${AGENTS_DIR}/analyzer.md" <<'ANALYZERMD'
---
name: analyzer
...
if [ -f "${AGENTS_DIR}/reviewer.md" ]; then
  ...
  echo "▶ 구버전 reviewer.md 제거 (검증은 Codex로 대체)"
fi
...
# model = "gpt-5.4-mini"        # 비용 절감용 소형 모델 예시
